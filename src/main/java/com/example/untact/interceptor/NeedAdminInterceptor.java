package com.example.untact.interceptor;
...
public class NeedAdminInterceptor implements HandlerInterceptor {
	...
	if (isAdmin == false) {
		...
		response.getWriter().append("location.replace('/adm/member/login?redirectUrl=" + request.getAttribute("encodedAfterLoginUrl") + "');");
		response.getWriter().append("</script>");
